...
class DonateController extends Controller
{
    ...
    public function index($url = '')
    {
        ...
        $query = DB::table('mamount')
            ->select('mamount.ID', 'mamount.Amount', 'mprogramdtl.Information')
            ->join('mprogramdtl', 'mamount.ID', '=', 'mprogramdtl.IDAmount')
            ->join('mprogram', 'mprogramdtl.IDProgram', '=', 'mprogram.ID');
        if (!empty($app['program'])) {
            $query->where('mprogram.ID', $app['program']->ID);
        } else {
            $query->groupBy('mamount.ID');
        }
        $app['amount'] = $query->get()->toArray();
