                ...
                <div class="form-group mt-6 select2-method">
                    ...
                    <div class="px-1 py-3 border-custom block bg-orange-100 rounded border-orange-200">
                        <div class="flex items-center justify-between">
                            ...
                            <a href="javascript:getPayment({{ $bank[0]->ID }});" onclick="BackTo(&#39;payment&#39;)" class="rounded-full py-2 px-4 text-white bg-orange-600">Ubah 
                ...
                <div class="donate-form-login">
                    @if(!empty(Session::get('UserID')))
                        <p class="mb-3 text-black text-sm">
                            <strong class="block">{{ Session::get('UserName') }}</strong>
                            <span class="color text-xs">{{ Session::get('UserEmail') }} - {{ Session::get('UserPhone') }}</span>
                        </p>
                        <input type="hidden" name="donorID" value="{{ Session::get('UserID') }}">
                        <input type="hidden" name="name" value="{{ Session::get('UserName') }}">
                        <input type="hidden" name="phone" value="{{ Session::get('UserPhone') }}">
                        <input type="hidden" name="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" value="{{ Session::get('UserEmail') }}">
                        <input type="hidden" name="ref" value="{{ $ref }}">
                    @else
                        <input type="hidden" name="ref" value="{{ $ref }}">
                        <p class="text-black text-sm text-center mb-6">
                            <a href="{{ url('/login') }}" class="text-blue-500 underline">Login</a> atau lengkapi data berikut
                        </p>
