...
<body class="detail-program">
    ...
    <main class="main-content">
        <header id="header-program-title" class="container flex items-center px-2 py-2 justify-between">
            ...
            <span class="title-program-header flex-1 font-semibold text-sm truncate">{{ $Program->Name }}</span>
            ...
            <div class="container max-w-56rem px-1 pb-6 px-4 md:pb-10 md:pt-10 mx-auto">
                <figure class="program-cover progressive mb-5 overflow-hidden">
                    <img src="{{ url('photos/program/'.$Program->Banner) }}" data-src="{{ url('photos/program/'.$Program->Banner) }}?ar=16:9&amp;w=720&amp;fit=crop&amp;auto=format,compress" alt="{{ $Program->Name }}"><noscript>
                        <img src="{{ url('photos/program/'.$Program->Banner) }}?ar=16:9&amp;w=720&amp;fit=crop&amp;auto=format,compress" alt="Cover {{ $Program->Name }}" /></noscript>
                </figure>
                <h1 class="text-lg sm:text-xl md:text-3xl font-bold text-gray-800 leading-normal">{{ $Program->Name }}</h1>
                <div class="program-head">
                    <div class="md:flex md:-mx-4 items-center">
                        <div class="md:w-3/4 md:px-4">
                            <p class="program-headline text-gray-600 py-2 text-xs sm:text-sm md:text-xl">{{ $Program->Summary }}
                        </div>
                    </div>
                        ...
                        <div class="toggleDonation ">
                            ...
                            <div class="program-collected my-4">
                                <div class="program-collected__bar bg-gray-custom w-full h-2 rounded overflow-hidden">
                                    <div class="program-collected__progress bg-orange-900 h-2 max-w-full" style="width: {{ $Program->Percent != '' ? $Program->Percent : 100 }}%"></div>
                                </div>
                                <div class="p-card__count mt-1">
                                    <div class="p-card__countitem">
                                        <div class="p-card__stats"><span class="text-xs md:text-xl color-gray">Donasi</span><span class="font-bold text-black text-sm md:text-3xl">{{ $Program->CountDonate }}</span></div>
                                    </div>
                                    <div class="p-card__countitem text-right">
                                        <div class="p-card__stats"><span class="text-xs md:text-xl color-gray">{{ $Program->DaysLeft != '' ? 'Hari Lagi' : 'Status' }}</span><span class=" text-black text-sm md:text-3xl font-bold">{{ $Program->DaysLeft != '' ? $Program->DaysLeft : '∞' }}</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="w-full md:w-1/2 text-right px-4 py-4 md:pt-4 md:px-0 mb:pb-0 sm:flex md:block">
                            ...
                            <div class="w-full sm:w-1/2 md:w-auto md:inline-block">
                                <a class="border border-blue-800 block sm:float-right md:float-none md:px-4 mr-2 mt-1 px-2 py-2 rounded share-link-popup text-blue-800 text-center text-xs md:text-sm" href="https://www.facebook.com/sharer/sharer.php?u={{ url('/') }}">
                            ...
                            <div class="tab-content">
                                            ...
                                            <div class="w-full">
                                                ...
                                                <div class="contentfundraiser mb-5">
                                                    <div class="flex bg-white md:px-4 md:py-4 pb-4 mb-4 md:rounded border-b-custom last:border-b-0 last:mb-0 md:border-custom ">
                                                        ...
                                                        <div class="text-sm">
                                                            <h3 class="text-gray-900 leading-none text-sm md:text-base mb-1">{{ $row->Name }}</h3>
                                                            <div class="color-gray text-xs md:text-sm mb-1 color-gray">Berhasil mengajak {{ $row->CountDonor }} orang untuk berdonasi</div>
                                                ...
                                                <div class="md:bg-gray-custom border-custom p-2 rounded-sm shadow:md md:p-6" id="FundraiserBtn">
                                                    <div class="flex -mx-1 md:-mx-4 items-center">
                                                        ...
                                                        <div class="w-2/6 md:w-1/2 px-1 md:px-4">
                                                            <a class="block rounded bg-blue-500 hover:bg-blue-600 px-1 py-2 md:px-4 md:py-6 text-white font-bold text-center text-xs md:text-xl no-underline" href="{{ url('/fundraiser/'.$Program->ID) }}">Jadi Fundraiser <span class="hidden md:inline-block">Sekarang Juga</span></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div id="tab-news" class="tab-pane">
                                    ...
                                    <div class="timeline">
                                        ...
                                        <div class="entry">
                                            <div class="title"><span class="text-xs md:text-md">{{ $row->ProgressDate }}</span>
                                            </div>
                                            <div class="body">
                                                <p class="text-sm md:text-lg">{{ $row->Summary }}
                                    ...
                                    <div id="tab-donatur" class="tab-pane">
                                        <h3 class="md:hidden mb-5 text-xl font-bold">Donatur ({{ $Program->CountDonate }})</h3>
                                        <div class="flex flex-wrap donatur-list">
                                            ...
                                            <div class="w-full">
                                                <div class="flex items-center bg-white shadow-sm rounded-sm mb-2 px-4 py-4 rounded">
                                                    ...
                                                    <div class="text-sm">
                                                        <p class="text-gray-900 leading-none">{{ $row->Name }}
                                                        <p class="text-orange-900 font-bold">Sebesar Rp
                                                            @php
                                                            $format_rupiah = number_format($row->Amount,0,',','.');
                                                            echo $format_rupiah;
                                                            @endphp
                                                        <p class="text-gray-500 text-xs italic">{{ $TimeSince[$key] }}
                                                        <p class="text-gray-700 text-xs italic mt-2">"{{ $row->Message }}"
                                                    </div>
                                                </div>
                                            </div>
                                            @endforeach
