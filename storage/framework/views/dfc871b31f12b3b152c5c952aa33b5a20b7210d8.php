...
<body>
  ...
  <script>
    ...
    //   maxFilesize: (<?php echo e(lcfirst(str_singular(request('type') ?: '')) == 'image' ? config('lfm.max_image_size') : config('lfm.max_file_size')); ?> / 1000)
        acceptedFiles: "<?php echo e(lcfirst(str_singular(request('type') ? request('type') : '')) == 'image' ? implode(',', config('lfm.valid_image_mimetypes')) : implode(',', config('lfm.valid_file_mimetypes'))); ?>",
        maxFilesize: (<?php echo e(lcfirst(str_singular(request('type') ? request('type') : '')) == 'image' ? config('lfm.max_image_size') : config('lfm.max_file_size')); ?> / 1000)
    }
  </script>
</body>
</html>
